...
import antaAgentExtension from "../src/anta-agent-extension.ts";
import { AntaAgent } from "../src/index.ts";

/**
 * anta-agent adapter extension against the shared suite harness. PI_CODING_AGENT_DIR
 * is redirected to a temp dir so the adapter's memory.json and data directory
 * never touch the real home directory.
 */
describe("anta-agent built-in memory extension", () => {
	...
	function setupAgentDir(): string {
		const agentDir = mkdtempSync(join(tmpdir(), "anta-agent-agentdir-"));
	...
	it("registers tools and commands and persists captured turns to L0", async () => {
		...
		const seeder = new AntaAgent({ config: { dataDir: join(agentDir, "memory-data") }, runner: async () => "" });
		await seeder.initialize();

		const harness = await createHarness({ extensionFactories: [antaAgentExtension] });
	...
	it("injects recalled memories into the provider request without polluting the session", async () => {
		const agentDir = setupAgentDir();
		const dataDir = join(agentDir, "memory-data");
		const seeder = new AntaAgent({ config: { dataDir }, runner: async () => "" });
		await seeder.initialize();
		await seeder.remember("The user prefers tabs over spaces", { sessionKey: "seed" });

		const harness = await createHarness({
			extensionFactories: [antaAgentExtension],
		});
		harnesses.push(harness);
