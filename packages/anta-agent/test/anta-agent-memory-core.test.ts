...
import type { AntaAgentConfigInput } from "../src/config.ts";
import { loadAntaAgentConfigFile, parseAntaAgentConfig } from "../src/config.ts";
import { AntaAgent } from "../src/core/anta-agent-core.ts";

function write(path: string, content: string): void {
	writeFileSync(path, content);
}

describe("parseAntaAgentConfig", () => {
	...
	it("keeps a valid explicit dataDir", () => {
		const config = parseAntaAgentConfig({ dataDir: "/tmp/anta-agent-x" });
		expect(config.dataDir).toBe("/tmp/anta-agent-x");
	});

	it("defaults the data dir to the agent dir from PI_CODING_AGENT_DIR", () => {
		process.env.PI_CODING_AGENT_DIR = "/tmp/anta-agent-agentdir";
		try {
			expect(parseAntaAgentConfig().dataDir).toBe("/tmp/anta-agent-agentdir/memory");
			process.env.PI_CODING_AGENT_DIR = "";
			expect(parseAntaAgentConfig().dataDir).toBe(join(homedir(), ".pi", "agent", "memory"));
		} finally {
	...
	it("reads JSON config files, ignoring missing or invalid ones", () => {
		expect(loadAntaAgentConfigFile(join(tmpdir(), "anta-agent-missing-memory.json"))).toEqual({});
		const file = join(tmpdir(), `anta-agent-memory-${Date.now()}.json`);
		write(file, JSON.stringify({ promptMode: "chat", recall: { maxResults: 7 } }));
		const input: AntaAgentConfigInput = loadAntaAgentConfigFile(file);
		const config = parseAntaAgentConfig(input);
		expect(config.promptMode).toBe("chat");
		expect(config.recall.maxResults).toBe(7);
	});
});

describe("AntaAgent end-to-end", () => {
	const dirs: string[] = [];
	...
	function makeMemoryDir(): string {
		const dir = mkdtempSync(join(tmpdir(), "anta-agent-e2e-"));
		dirs.push(dir);
		return dir;
	}
		...
		return async (params: { systemPrompt: string; userPrompt?: string }) => {
			if (/情境切分/.test(params.systemPrompt)) {
				return JSON.stringify([
					{
						scene_name: "正在与用户推进 anta-agent 开发",
						message_ids: [],
						memories: [
							{
								content: "用户正在为 pi 实现四层记忆系统 anta-agent",
								type: "work_fact",
								priority: 85,
								source_message_ids: [],
								metadata: {},
	...
	it("captures turns, extracts memories via L1, and recalls them", async () => {
		...
		const memory = new AntaAgent({ config: { dataDir }, runner: extractionRunner() });
			...
			messages: [
				{
					id: "",
					role: "user",
					content: "我们正在实现 anta-agent 四层记忆系统，请记住这个决定",
					timestamp: Date.now(),
				},
				{
					id: "",
					role: "assistant",
					content: "好的，已记录 anta-agent 四层记忆系统这个工作目标",
					timestamp: Date.now(),
				},
			],
		});
		...
		expect(hits[0]!.content).toContain("anta-agent");

		const recall = await memory.recall("anta-agent 记忆系统目前进展如何？");
	...
	it("dedupes repeated facts instead of accumulating duplicates", async () => {
		...
		const memory = new AntaAgent({ config: { dataDir }, runner });
		await memory.initialize();
		const turn = {
			sessionKey: "sess-dedup",
			messages: [
				{ id: "", role: "user" as const, content: "anta-agent 的四层记忆系统进展如何？", timestamp: Date.now() },
