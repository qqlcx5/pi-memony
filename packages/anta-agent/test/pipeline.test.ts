...
describe("extractMemories", () => {
	const config = parseAntaAgentConfig({ promptMode: "code" });
	...
	it("parses scene segmentation and validates candidates", async () => {
		const { runner } = scriptedRunner([
			{
				...
				body: () =>
					JSON.stringify([
						{
							scene_name: "正在围绕 anta-agent 推进存储层开发",
							message_ids: ["m1", "m2"],
							memories: [
								{
									content: "用户决定 anta-agent 使用 node:sqlite 做存储",
									type: "work_fact",
									priority: 90,
		...
		expect(scenes[0]!.sceneName).toContain("anta-agent");
	...
	it("caps memories across scenes at maxMemoriesPerSession", async () => {
		const config = parseAntaAgentConfig({ promptMode: "code", extraction: { maxMemoriesPerSession: 3 } });
		const memory = {
			content: "x",
			type: "work_fact",
			priority: 80,
...
describe("dedupMemories", () => {
	it("parses batch decisions and defaults undecided candidates to store", async () => {
		const config = parseAntaAgentConfig({ promptMode: "chat" });
		const { runner } = scriptedRunner([
			{
				match: /冲突检测/,
				body: () =>
		...
		const store = new SqliteMemoryStore(join(tmpdir(), `anta-agent-dedup-${Date.now()}.db`));
		store.insertMemory({
			id: "old-1",
			content: "old",
			type: "persona",
	...
	it("degrades to store-all when the dedup output is unparsable", async () => {
		...
		const store = new SqliteMemoryStore(join(tmpdir(), `anta-agent-dedup-degrade-${Date.now()}.db`));
		const extracted = [
			{
				recordId: "nm-1",
	...
	it("falls back to the candidate priority when merged_priority is omitted", async () => {
		...
		const store = new SqliteMemoryStore(join(tmpdir(), `anta-agent-dedup-prio-${Date.now()}.db`));
		const extracted = [
			{
				recordId: "nm-1",
	...
	it("degrades a malformed update decision to store instead of dropping the candidate", async () => {
		...
		const store = new SqliteMemoryStore(join(tmpdir(), `anta-agent-dedup-malformed-${Date.now()}.db`));
		const extracted = [
			{
				recordId: "nm-1",
				content: "candidate fact",
...
describe("scene extraction", () => {
	it("applies create/update/delete ops and rebuilds the index", async () => {
		const dir = mkdtempSync(join(tmpdir(), "anta-agent-scene-"));
		const paths = storagePaths(dir);
		const config = parseAntaAgentConfig({ dataDir: dir });
		const { runner } = scriptedRunner([
			{
				...
				body: () =>
					JSON.stringify([
						{
							action: "create",
							file: "技术研究 anta-agent.md",
							summary: "anta-agent 记忆系统开发",
							heat: 1,
							content:
								"-----META-START-----\ncreated: now\nupdated: now\nsummary: anta-agent 记忆系统开发\nheat: 1\n-----META-END-----\n\n## 核心特征\n本地 SQLite 存储方案。",
						},
		...
		const memories = [
			{
				id: "m1",
				content: "anta-agent 使用本地 SQLite",
				type: "work_fact" as const,
				priority: 80,
				sceneName: "s",
				sourceMessageIds: [],
		...
		expect(result.entries[0]!.file).toBe("技术研究-anta-agent.md");
		expect(existsSync(join(paths.sceneBlocksDir, "技术研究-anta-agent.md"))).toBe(true);
		expect(readSceneIndex(paths)).toHaveLength(1);

		const ops = parseSceneOps(JSON.stringify([{ action: "delete", file: "技术研究-anta-agent.md" }]));
		const after = applySceneOps(paths, ops);
		expect(after).toHaveLength(0);
		expect(existsSync(join(paths.sceneBlocksDir, "技术研究-anta-agent.md"))).toBe(false);
		rmSync(dir, { recursive: true, force: true });
	});

	it("rebuilds a corrupt scene index from META headers", () => {
		const dir = mkdtempSync(join(tmpdir(), "anta-agent-scene-rebuild-"));
		const paths = storagePaths(dir);
...
describe("persona generation", () => {
	it("generates on first run and writes persona.md with backups", async () => {
		const dir = mkdtempSync(join(tmpdir(), "anta-agent-persona-"));
		const paths = storagePaths(dir);
		const config = parseAntaAgentConfig({ dataDir: dir, promptMode: "chat" });
		let first = true;
		const { runner } = scriptedRunner([
			{
...
describe("PipelineManager scheduling", () => {
	function makeManager(config: ReturnType<typeof parseAntaAgentConfig>, runner: LlmRunner) {
		const dir = mkdtempSync(join(tmpdir(), "anta-agent-manager-"));
		const paths = storagePaths(dir);
	...
	it("fires L1 at warmup thresholds 1,2,4 then converges on everyNConversations", async () => {
		const config = parseAntaAgentConfig({
