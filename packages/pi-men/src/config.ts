...
export interface PiMenConfigInput {
	dataDir?: string;
 ...
 */
export function parsePiMenConfig(input?: PiMenConfigInput): PiMenConfig {
	const i = input ?? {};
	return {
...
/** Read a pi-men JSON config file; returns {} on missing/invalid files. */
export function loadPiMenConfigFile(filePath: string): PiMenConfigInput {
	try {
		const raw = readFileSync(filePath, "utf8");
		const parsed = JSON.parse(raw);
		return typeof parsed === "object" && parsed !== null ? (parsed as PiMenConfigInput) : {};
	} catch {
		return {};
	}
}
