...
/**
 * Built-in pi-men memory extension: captures turns into the local four-layer
...
const factory: ExtensionFactory = (pi: ExtensionAPI) => {
	...
	const getMemory = (ctx: ExtensionContext): PiMen => {
		if (!memory) {
			...
			memory = new PiMen({
				...
				logger: {
					debug: (message) => safeEmit("pi-men:debug", { message }),
					info: (message) => safeEmit("pi-men:info", { message }),
					warn: (message) => safeEmit("pi-men:warn", { message }),
					error: (message) => safeEmit("pi-men:error", { message }),
				},
			});
		}
		return memory;
	...
	pi.registerCommand("memory", {
		description: "Show pi-men memory status",
		handler: async (_args, ctx) => {
			try {
				const client = getMemory(ctx);
				const stats = client.stats();
				const lines = [
					"pi-men memory status:",
				...
				ctx.ui.notify(`pi-men status failed: ${errorMessage(error)}`, "error");
			}
		},
	});
};
