...
import { getAgentDir as getDefaultAgentDir, getDisplayAppName, getSessionsDir } from "../config.ts";
		...
		if (existsSync(this.sessionFile)) {
			...
			if (this.fileEntries.length === 0) {
				...
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid ${getDisplayAppName()} session: ${explicitPath}`);
				}
				this.newSession();
